...
public class FastCollinearPoints {
    ...
    private ArrayList<Double> slopesBefore;
    private final List<LineSegment> segments = new ArrayList<>();
    
    public FastCollinearPoints(Point[] points) {
        slopesBefore = new ArrayList<Double>();
    ...
    private void addSegmentIfNew(List<Point> slopePoints, double slope) {
        ...
        for (double currentEndPoint : slopesBefore) {
            if (0 == Double.compare(slope, currentEndPoint)) {
                 return;
             }
        }
        segments.add(new LineSegment(startPoint, endPoint));
        slopesBefore.add(slope);
...
//        }
    ...
    public static void main(String[] args)
    {
       ...
       // In in = new In(args[0]);
        In in = new In("collinear/inputtx.txt"); //本地测试使用
        int n = in.readInt();
